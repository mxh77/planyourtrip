...
# Installe l'APK release sur le téléphone connecté en USB.
# Prérequis : avoir déjà buildé avec release-build.sh au moins une fois.
...
if [ -z "$APK" ]; then
  echo -e "${RED}✗ Aucun APK release trouvé. Lance d'abord ./release-build.sh${RESET}"
  exit 1
fi

echo -e "\n${YELLOW}══════════════════════════════════════════${RESET}"
echo -e "${YELLOW}  Mon Petit Roadtrip — Install Release APK  ${RESET}"
echo -e "${YELLOW}══════════════════════════════════════════${RESET}\n"
